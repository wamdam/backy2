...
The enforcement policy is specified by a comma separated list of time categories:
...
You can call ``benji enforce`` as many times a day as you want. If no version currently
falls into a specified category the oldest version of the category below is always kept,
so that it has a chance to get old enough. So if in third example there is one version
that is older than 24 hours but younger than one year it will be kept till it is old
enough even though it falls out of the first specified category.
